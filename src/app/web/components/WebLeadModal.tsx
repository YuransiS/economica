...
export default function WebLeadModal({
  ...
  // Autofill from localStorage and check registration status
  useEffect(() => {
    if (isOpen) {
      const savedName = localStorage.getItem('user_name');
      const savedPhone = localStorage.getItem('user_phone');
      const savedTelegram = localStorage.getItem('user_telegram');
      const alreadyRegistered = localStorage.getItem('is_webinar_registered');
      
      if (savedName) setName(savedName);
      if (savedPhone) setPhone(savedPhone);
      if (savedTelegram) setTelegram(savedTelegram);
      if (alreadyRegistered === 'true') setIsRegistered(true);
    }
  }, [isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success && result.redirectUrl) {
        ...
        localStorage.setItem('is_webinar_registered', 'true');
            ...
            >
              ...
              {isRegistered ? (
                <div className="flex flex-col items-center py-6">
                  <h3 className="mb-8 font-montserrat text-2xl font-bold uppercase text-center text-[#4E0000]">
                    Ви вже зареєстровані!
                  </h3>
                  <a
                    href="https://telegram.me/+omVaRnq7pqVjNzNi"
                    target="_blank"
