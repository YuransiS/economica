...
export default function CheckoutClient({ payload }: { payload: any }) {
  ...
  useEffect(() => {
    console.log('Checkout page opened for Economica:', payload.n, payload.t);
  }, [payload]);
    ...
    try {
      const res = await fetch('/api/create-payment', {
        ...
        body: JSON.stringify({
          ...
          visitor_id: payload.u,
          successUrl: `${window.location.origin}/api/wayforpay/return`,
      ...
      wayforpay.run(
        ...
        function (response: any) {
          ...
          setStatus('approved');
          setIsLoading(false);
          // Redirect to the checking payment route that Sofia platform has
          router.push(`/api/wayforpay/return?order=${paymentData.orderReference}&tariff=${encodeURIComponent(payload.t)}`);
        },
        function (response: any) {
  ...
  return (
    <div className="min-h-screen bg-[#07080a] flex flex-col items-center justify-center p-4">
      ...
      <div className="max-w-md w-full bg-white/5 border border-white/10 p-8 rounded-2xl flex flex-col items-center relative overflow-hidden">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[150%] h-[150px] bg-blue-600/10 blur-[100px] pointer-events-none" />
        
        <h1 className="text-white text-2xl font-bold mb-2 text-center relative z-10 font-black uppercase tracking-tight">Оформлення замовлення</h1>
        ...
        <div className="w-full bg-black/40 rounded-xl p-4 mb-8 space-y-4 relative z-10 border border-white/5">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-blue-400 font-bold text-xl">{payload.a} {payload.c}</span>
          </div>
        </div>
        ...
        <button
          onClick={handlePay}
          disabled={isLoading}
          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-6 rounded-xl transition-all flex items-center justify-center gap-2 relative z-10 disabled:opacity-50"
        >
          {isLoading ? (
